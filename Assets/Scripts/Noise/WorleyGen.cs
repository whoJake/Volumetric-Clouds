...
{
    static ComputeShader generatorShader = Resources.Load<ComputeShader>("WorleyCompute");
    ...
    public static RenderTexture Generate2DGPU(int textureSize, int seed, int frequency) {
        Vector2[,] points = new Vector2[frequency, frequency];
        ...
        for (int y = 0; y < frequency; y++) {
            for (int x = 0; x < frequency; x++) {
                points[x, y] = new Vector2(Random.Range(0f, 1f), Random.Range(0f, 1f));
            }
        }

        ComputeBuffer pointBuffer = new ComputeBuffer(frequency * frequency, sizeof(float) * 2);
        ...
        generatorShader.SetBuffer(0, "_RandomizedPoints2D", pointBuffer);
    ...
    public static RenderTexture Generate3DGPU(int textureSize, int seed, int frequency) {
        Vector3[,,] points = new Vector3[frequency, frequency, frequency];
        ...
        for (int z = 0; z < frequency; z++) {
            for (int y = 0; y < frequency; y++) {
                for (int x = 0; x < frequency; x++) {
                    points[x, y, z] = new Vector3(Random.Range(0f, 1f), Random.Range(0f, 1f), Random.Range(0f, 1f));
                }
            }
        }

        ComputeBuffer pointBuffer = new ComputeBuffer(frequency * frequency * frequency, sizeof(float) * 3);
        pointBuffer.SetData(points);
        ...
        generatorShader.SetBuffer(1, "_RandomizedPoints3D", pointBuffer);
        generatorShader.SetInt("texture_size", textureSize);
        generatorShader.SetInt("frequency", frequency);
    ...
    #region CPU Worley Implementation
